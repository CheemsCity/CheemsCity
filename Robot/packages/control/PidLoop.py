...
while True:
    ...
    if result != -1:
        combo = birdview.Visual(image, skyview,
        ...
        comboBig = cv2.rectangle(comboBig, (50, int(480 - (powerLeft * 3))),
                                 (100, 480), (255, 0, 0), 5)
        comboBig = cv2.rectangle(comboBig,
                                 ((640 - 100), int(480 - (powerRight * 3))),
                                 (640 - 50, 480), (255, 0, 0), 5)
        comboBig = cv2.line(comboBig, (320, 480), (320, 0), (0, 0, 255), 5)
        comboBig = cv2.putText(comboBig,
                               "distanza: " + str(result['Center_distance']),
                               (5, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                               (255, 255, 0), 2, 1)
        comboBig = cv2.putText(comboBig, "error De = " + str(pid.Derivative),
                               (5, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                               (255, 255, 0), 2, 1)
        comboBig = cv2.putText(comboBig, "error I = " + str(pid.Integral),
                               (5, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                               (255, 255, 0), 2, 1)
        cv2.imshow("frame", comboBig)
        key = cv2.waitKey(1) & 0xFF

        print("[INFO] velocita motore sinistro = " + str(powerLeft) +
              " , motore destro = " + str(powerRight))
