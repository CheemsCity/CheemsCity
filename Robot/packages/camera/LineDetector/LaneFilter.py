...
class LaneFilter:
    ...
    def roi_to_height(self, img, height: int):
        ...
        if (height > self.height):
            raise ValueError(
                "[ERROR] Altezza inserita supera l'altezza della foto: {}".
                format(self.height))
        trapezio = np.array([[(0, self.height), (self.width, self.height),
    ...
    def custom_roi(self, img, height: int, wRight: int, wLeft: int):
        ...
        if (height > self.height):
            raise ValueError(
                "[ERROR] Altezza ROI superiore ad altezza massima foto: {} > {}"
                .format(height, self.height))
        if (wLeft < 0):
            raise ValueError(
                "[ERROR] wLeft ROI inferiore a 0: {} < 0".format(wleft))
        if (wRight > self.width):
            raise ValueError(
                "[ERROR] wRight ROI superiore a larghezza massima foto: {} > {}"
                .format(wRight, self.width))
        trapezio = np.array([[(0, self.height), (self.width, self.height),
                              (wRight, (self.height - height)),
                              (wLeft, (self.height - height))]])
        mask = np.zeros_like(img)
        cv2.fillPoly(mask, trapezio, 255)
        masked_image = cv2.bitwise_and(img, mask)
        return masked_image
