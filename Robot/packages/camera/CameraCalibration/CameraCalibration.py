...
class ChessboardApp:
    ...
    def __init__(self, VideoStream, outPath):
        ...
        self.root.geometry(
            self.settings['camera_calibration_canvasResolution'])
        #panel for image
        self.panel = None
            ...
            while not self.endEvent.is_set():
                ...
                if self.n < self.nThresh:
                    ...
                    chessboard = Chessboard(
                        nx=self.settings['camera_calibration_chessboardX'],
                        ny=self.settings['camera_calibration_chessboardY'],
                        frame=self.frame,
                        square_size=self.
                        settings['camera_calibration_squareSize'])  #metri
                    if chessboard.ret == True:
                        self.ctrl = True
                        self.chessboards.append(chessboard)
                        self.n += 1
                        self.pbar['value'] = (self.n * 100) / self.nThresh
                        cv2.drawChessboardCorners(self.frame, (8, 8),
                                                  chessboard.imgpoints,
                                                  chessboard.ret)
                        print(self.n)
                ...
                image = image.resize(
                    self.settings['camera_calibration_resizeImage'],
                    Image.ANTIALIAS)
    ...
    def SaveMatrix(self):
        ...
        ret, matrix, distortion_coef, rv, tv = cv2.calibrateCamera(
