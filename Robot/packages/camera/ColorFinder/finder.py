...
class ColorFinder():
    ...
    def mat_dist(self):
        ...
        self.Md = np.sqrt(self.Mdiff[:, :, 0] + self.Mdiff[:, :, 1] +
                          self.Mdiff[:, :, 2])
    ...
    def defineRectangularContour(self):
        self.SO = (math.ceil(
            min(np.nonzero(self.bool_Md)[1]) * (1 - self.risk_coeff)),
                   math.ceil(
                       max(np.nonzero(self.bool_Md)[0]) *
                       (1 + self.risk_coeff)))
        self.NE = (math.ceil(
            max(np.nonzero(self.bool_Md)[1]) * (1 + self.risk_coeff)),
                   math.ceil(
                       min(np.nonzero(self.bool_Md)[0]) *
                       (1 - self.risk_coeff)))
    ...
    def defineRectangularContourCustom(self, customImage):
        try:
            self.SO = (math.ceil(
                min(np.nonzero(customImage)[1]) * (1 - self.risk_coeff)),
                       math.ceil(
                           max(np.nonzero(customImage)[0]) *
                           (1 + self.risk_coeff)))
            self.NE = (math.ceil(
                max(np.nonzero(customImage)[1]) * (1 + self.risk_coeff)),
                       math.ceil(
                           min(np.nonzero(customImage)[0]) *
                           (1 - self.risk_coeff)))
        except:
            return [None, None]
        #prendo i 2 punti che costruiscono il rettangolo più grande che contiene l'oggetto
